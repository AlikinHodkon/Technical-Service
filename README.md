...
`200`, `201` (с заголовком `Location`), `204`, `400` (ошибка валидации), `403` (CORS), `404`, `409` (конфликт: дубль `serialNumber`, недопустимый переход статуса, удаление оборудования с открытыми заявками), `429` (rate limit), `503` (внешний погодный API недоступен). `422` в API не используется — вся валидация тела/query единообразно отдаёт `400`.
